...
export type OAuthReturn={type:typeof OAUTH_RETURN;code?:string;error?:string};
...
export function parseOAuthReturn(href:string):OAuthReturn|null{
  ...
  if(url.searchParams.has("code")) {
    ...
    return code.length>0&&code.length<=2048?{type:OAUTH_RETURN,code}:{type:OAUTH_RETURN,error:"invalid_callback"};
  }
  if(url.searchParams.has("error")||hash.has("error"))return {type:OAUTH_RETURN,error:(url.searchParams.get("error")||hash.get("error"))==="access_denied"?"access_denied":"provider_error"};
  if(hash.has("access_token")||hash.has("refresh_token"))return {type:OAUTH_RETURN,error:"restart_required"};
  return null;
}
